...
import { getExchangeCopy, isPublicLocale, localePath, localizeAttribute, localizeCharacter, pick } from "@/components/acg-locale";
...
export default async function CharacterPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const trendBars = Array.from({ length: 18 }, (_, index) => 28 + ((character.circulatingUnits + index * 17) % 66));
  ...
  return <div className="exchange-page pb-36 lg:pb-16">
    ...
    <section className="exchange-panel overflow-hidden bg-[#111827] text-white">
      <div className="grid lg:grid-cols-[1.1fr_.9fr]">
        <div className="group relative min-h-[560px] lg:min-h-[760px]"><CharacterArt character={character} className="absolute inset-0 h-full" priority sizes="(min-width: 1024px) 58vw, 100vw" /><div className="absolute left-5 top-5 z-10 flex flex-wrap gap-2 sm:left-8 sm:top-8"><Badge tone={character.rightsType === "ORIGINAL" ? "warm" : "cool"}>{character.rightsType === "ORIGINAL" ? copy.common.original : copy.common.metadata}</Badge>{character.releaseSeason ? <Badge>{character.releaseSeason}</Badge> : null}</div></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_430px]">
      <section className="exchange-panel p-6 sm:p-8"><SectionHeading eyebrow={page.trendEyebrow} title={page.trendTitle} description={page.trendBody} /><div className="mt-8 grid h-56 grid-cols-[repeat(18,1fr)] items-end gap-1 rounded-[24px_6px_24px_6px] bg-[#111827] p-5">{trendBars.map((height, index) => <span key={index} className="rounded-t-full bg-gradient-to-t from-[#ff4e72] to-[#ffcc66]" style={{ height: `${height}%`, opacity: .45 + index / 32 }} />)}</div><div className="mt-4 flex items-center justify-between text-[10px] font-black uppercase tracking-[.16em] text-slate-400"><span>Signal open</span><span>Now · {currencyLabel(view.quote)}</span></div></section>
    ...
    <section className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
      ...
      <div className="exchange-panel p-6 sm:p-8"><SectionHeading eyebrow={page.rightsEyebrow} title={page.rightsTitle} description={page.rightsBody} /><div className="mt-7 grid gap-3"><div className="flex gap-3 rounded-[18px_5px_18px_5px] bg-[#eef8f7] p-4"><ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-[#19757a]" /><p className="text-sm leading-7 text-slate-600">{character.rightsType === "ORIGINAL" ? pick(locale, "Original first-party character and demo asset pack.", "平台原創角色與自有示範素材，可在公開版使用。") : pick(locale, "Metadata-only listing. Official art, voice, logos, and comic pages are not bundled.", "只展示資料型角色資訊，不內含官方立繪、語音、標誌或漫畫頁。")}</p></div>{view.sourceAttribution ? <a href={view.sourceAttribution.sourceUrl} target="_blank" rel="noreferrer" className="exchange-button-secondary justify-self-start"><ExternalLink className="h-4 w-4" />{page.source}</a> : null}</div></div>
    </section>
