...
export default async function MarketPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [sourceCharacters, watchlistIds] = await Promise.all([listCharacters({ search: search || undefined, tag: tag || undefined, rightsType: rightsType || undefined, locale }), getWatchlistIds().catch(() => [] as string[])]);
  ...
  characters = [...characters].sort((a, b) => sort === "new" ? (b.releaseSeason ?? "").localeCompare(a.releaseSeason ?? "") : sort === "quote" ? b.basePrice - a.basePrice : b.supporterCount - a.supporterCount);
  ...
  const copy = locale === "zh-Hant" ? { eyebrow: "SIGNAL DIRECTORY", title: "找到今天最對頻的角色。", body: "按作品季別、角色氣質、陪伴需求與來源狀態探索。熱度只代表喜愛正在流動，不建立輸家榜。", search: "搜尋角色、作品或關鍵字", tag: "標籤，例如 comfort", allRights: "所有來源", original: "原創角色", metadata: "資料型角色", allSeasons: "所有季別", allMoods: "所有氣質", sortSupport: "最多應援", sortQuote: "目前價格", sortNew: "最新季番", watching: "只看關注中", apply: "更新訊號", found: `找到 ${cards.length} 個訊號`, empty: "目前沒有符合條件的角色訊號，試著放寬篩選。" } : { eyebrow: "SIGNAL DIRECTORY", title: "Find the character on your frequency today.", body: "Explore by season, mood, comfort need, and source status. Heat only shows affection in motion; it never creates a loser board.", search: "Search character, series, or keyword", tag: "Tag, e.g. comfort", allRights: "All source types", original: "Original characters", metadata: "Metadata signals", allSeasons: "All seasons", allMoods: "All moods", sortSupport: "Most supported", sortQuote: "Current quote", sortNew: "Newest season", watching: "Watching only", apply: "Tune signals", found: `${cards.length} signals found`, empty: "No character signal matches these filters yet. Try opening the range." };
  ...
  return <div><SeasonTicker locale={locale} /><div className="exchange-page">
    ...
    {cards.length ? <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">{cards.map(({ character, comments }) => <CharacterCard key={character.id} character={character} watching={watchlistIds.includes(character.id)} commentCount={comments} locale={locale} />)}</div> : <div className="exchange-panel p-10 text-center text-slate-500">{copy.empty}</div>}
  </div></div>;
}
