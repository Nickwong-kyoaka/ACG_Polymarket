...
export function MissionPanel({ missions, locale }: { missions: DailyMissionView[]; locale: "en" | "zh-Hant" }) {
  ...
  return <div className="grid gap-3">{missions.map((mission) => { const Icon = icons[mission.key]; return <div key={mission.key} className="flex items-center gap-4 border-b border-black/15 bg-[#f3ecdf] p-4"><span className="mission-check">{mission.completed ? <Check className="h-4 w-4" /> : <Icon className="h-4 w-4" />}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-slate-800">{mission.title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{mission.description}</p></div>{mission.completed && !mission.claimed ? <button type="button" onClick={() => claim(mission.key)} disabled={Boolean(busy) || pending} className="exchange-button-primary px-3 py-2 text-[10px]">+{mission.reward}</button> : <span className="text-xs font-black text-[#19757a]">{mission.claimed ? (locale === "zh-Hant" ? "已領取" : "CLAIMED") : `+${mission.reward}`}</span>}</div>; })}{message ? <p className="text-sm font-bold text-[#19757a]" role="status">{message}</p> : null}</div>;
}
