...
describe("Render deployment support", () => {
  it("declares a main-branch Node web service for the Next.js app", () => {
    ...
    expect(yamlCommand("startCommand")).toBe(
      "npx prisma migrate deploy && npm run db:bootstrap && npm run start -- -H 0.0.0.0 -p $PORT",
    );
    expect(yamlCommand("initialDeployHook")).toBe("npm run db:bootstrap");
  ...
  it("runs migrations at runtime and migration smoke tests in CI", () => {
    expect(dockerfile).toContain("npx prisma migrate deploy && npm run db:bootstrap && npm run start");
